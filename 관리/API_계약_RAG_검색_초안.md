...
```
문서번호   DOCFLOW-API-RAG-01
작성일자   2026. 8. 11.  ·  정정 2026. 8. 20.
작성자     김보현
상태       구현 완료 — 다만 초안과 여러 곳이 다르다. 아래 "정정" 절이 진실이다
           검색 SRH-001(PR 기존) · SRH-003(#38) · SRH-004(#39) · RAG-002-1(#40)
합칠 곳    관리/API_계약서_v2.md  3절 엔드포인트 목록  ← 아직 안 합쳤다
근거       관리/RAG_임베딩모델_선정_결과서.md (모델·차원 확정)
```

> **`API_계약서_v2.md` 에 검색 엔드포인트가 없다.** `RAG-04`·`RAG-08` 이 P1 인데
> 대응 경로가 비어 있고 `VIS-06` 통합 검색만 있다. `0011` 직후 구현 대상이라
> 프런트와 맞출 계약이 필요해 초안을 만들었다.
>
> **2026-08-20 현재도 `API_계약서_v2.md` 에는 여전히 없다.** 검색이 네 기능
> (`SRH-001`·`SRH-003`·`SRH-004`·`RAG-002-1`)까지 구현됐으므로 **합치는 것이 남은
> 일이다.** 그때 아래 초안 본문이 아니라 **정정 절을 옮겨야 한다.**
...
# ⚠ 정정 (2026-08-20) — 구현이 이 초안과 다르다

**아래 초안은 2026-08-11 설계 기록이고, 구현은 여러 곳에서 다르게 갔다.**
프런트나 다른 연동을 붙일 때는 **이 절을 진실로 본다.** 초안은 왜 그렇게 정했는지
읽는 용도로 남긴다.

`RAG` 2단 ID 는 v5 에서 3단으로 바뀌었다. 이 절은 v5 ID 를 쓴다
(대응은 `관리/기능명세서.md` 부록 A).

## 1. 경로가 달라졌다

| 초안 | **구현 (`main`)** | 왜 |
|---|---|---|
| `GET /api/projects/{project_id}/search` (`mode` 파라미터) | **`POST /api/search`** | 아래 ①② |
| — | **`POST /api/search/keyword`** | 새로 생김 |
| — | **`POST /api/search/hybrid`** ← **화면이 쓰는 것** | 새로 생김 |
| `GET /api/amount-precedents` | `GET /api/projects/{id}/amount-precedents` | 프로젝트 아래로 |
| `POST /api/admin/search-eval` | **만들지 않았다** | 컨테이너 안에서 도는 스크립트로 대신한다(`도구/check_hybrid_quality.py`) |

**① 경로에서 `project_id` 를 뺀 이유 — 쟁점 1 의 결론이 경로를 바꿨다.**
초안이 쟁점 1 에서 **B 안(내가 멤버인 프로젝트 전체)** 을 골랐다. 그런데 범위가
"멤버십 전체" 일 수 있으면 **경로에 프로젝트를 하나만 둘 수 없다.** 초안은 그
연결을 보지 못하고 경로는 프로젝트 아래에 두었다. 구현에서는 범위를 본문의
`project_ids` 로 받는다.

```
project_ids = null       내가 멤버인 모든 프로젝트
project_ids = [3]        그 프로젝트만
project_ids = [3, 7]     골라서 몇 개
```

빈 배열 `[]` 은 **거부한다**(422). "아무것도 검색하지 않겠다" 는 뜻이 모호해서,
전체를 원하면 `null` 을 명시하게 했다.

**② `GET` 이 아니라 `POST` 인 이유.** 질의가 문장이라 URL 이 길어지고, 범위가
배열이며, **검색어가 브라우저 이력과 접근 로그에 남지 않아야 한다** — 조달 문서에서
"무엇을 찾는지" 가 사업 정보다.

## 2. 초안이 걱정한 것에 대한 답

> 초안 1절: *"**R1 하나에 `mode` 를 둔다.** 따로 만들면 나중에 합칠 때 계약이 둘로
> 갈린다."*

**엔드포인트는 셋으로 나눴지만 계약은 갈리지 않았다.** 셋이 **같은 응답 스키마**
(`SearchResponse` · `SearchResultItem`)를 쓰고, 어느 방식으로 걸렸는지는 결과의
`match_kind` 로만 드러난다. 그래서 하이브리드가 두 결과를 한 순위로 합칠 때 변환이
필요 없다.

나눈 이유 — `의미 검색(SRH-001)` 이 이미 구현돼 프런트가 쓰고 있었다. 요청 스키마에
`mode` 를 더하면 **구현된 기능의 계약을 고치고 프런트도 같이 고쳐야 한다.** 셋을
따로 두면 서로를 재보고 비교할 수도 있다(품질 측정이 그렇게 한다).

## 3. 쟁점 셋의 결론

| | 초안 | 결론 |
|---|---|---|
| **쟁점 1** 스코프 | B 안 제안 | **B 채택.** 멤버 아닌 프로젝트를 지정하면 **`404`**(403 이 아니다 — 남의 프로젝트 존재 자체를 숨겨 id 훑기를 막는다) |
| **쟁점 2** 결과 단위 | 청크 단위 + `per_document` 파라미터 | **청크 단위 채택.** 다만 `per_document` **파라미터를 만들지 않았다** — 문서별로 묶어 접는 것은 **화면이 한다**(`SearchView` 의 `groupByDocument` · 문서당 2개만 펼침). 서버가 정할 일이 아니다 |
| **쟁점 3** 정렬 | 유사도 내림차순 | 의미·키워드는 각자 점수 내림차순. **하이브리드는 `RRF` 순** — 초안 시점에 없던 개념이다 |

## 4. 엔드포인트 셋 — 요청과 응답

### 공통 응답 `SearchResponse`

```
{
  "query": "대금은 언제 받을 수 있나요",
  "searched_project_ids": [3],
...
뜻이 없다.** 화면이 그 사실을 배너로 알린다.

### `SearchResultItem`

| 필드 | 뜻 |
|---|---|
| `chunk_id` · `document_id` · `document_filename` | 출처 |
| `project_id` · `project_name` | 범위가 여러 프로젝트일 때 필요 |
...
| `content_start` · `content_end` | `extracted_texts.content` 안의 구간. 원문 강조용. 모르면 `null` |

**키워드·하이브리드에서만 채워지는 필드** (의미 검색에서는 전부 `null` — 기존 응답과
호환된다)

| 필드 | 뜻 |
|---|---|
| `match_kind` | `"vector"` · `"keyword"` · `"both"` |
| `match_count` | 검색어가 그 조각에 몇 번 나오나 |
| `match_offset` | `snippet` 안에서 검색어가 시작하는 위치 |
| `fused_score` | RRF 점수 (하이브리드만) |
| `vector_rank` · `keyword_rank` | 각 방식에서 몇 등이었나 (하이브리드만) |

> **⚠ `similarity` 를 무조건 "유사도 N%" 로 표시하면 안 된다.**
>
> | `match_kind` | `similarity` 의 뜻 |
> |---|---|
> | `"vector"` · `"both"` · `null` | **코사인 유사도** (0~1, 1이 가장 가까움) |
> | `"keyword"` | **트라이그램 낱말 유사도** — 조사가 붙으면 1 미만 (실측: 정확일치 `1.0` · 조사붙음 `0.8` · 불일치 `0`) |
>
> 뜻이 다른 값을 같은 이름으로 부르면 안 된다. 화면은 코사인일 때만 백분율로
> 보여주고, 키워드로만 걸린 결과에는 대신 `match_count` 를 보여준다.

> **⚠ `fused_score` 는 유사도가 아니다.** 크기 자체에 뜻이 없고 **서로 비교할 때만**
> 뜻이 있다(약 0.016 부터). 백분율로 표시하면 안 된다.

> **⚠ `match_offset` 은 파이썬 코드포인트 기준이다.** JavaScript 처럼 UTF-16
> 코드유닛으로 세는 클라이언트는 BMP 밖 문자(이모지 · 확장 한자 `𠀋`)가 앞에 있으면
> 어긋나 **서로게이트 반쪽이 잘린다**(재현했다). 쓰기 전에 그 자리에 검색어가 있는지
> 확인하고, 어긋나면 스니펫에서 직접 찾는다. 스니펫 안의 첫 매치가 곧 그 자리다.

### `POST /api/search` — 의미 검색 (`SRH-001` 의미 검색)

```
{ "query": str(1~1000), "project_ids": [int]|null, "limit": 1~50 = 10,
  "document_id": int|null, "min_similarity": -1.0~1.0|null }
```

### `POST /api/search/keyword` — 키워드 검색 (`SRH-003` 키워드 검색)

본문에 검색어가 **그대로 있는** 조각만 찾는다. 문서번호·고유명사·금액용.

```
{ "query": str(1~200), "project_ids": [int]|null, "limit": 1~50 = 10,
  "document_id": int|null }
```

- **`min_similarity` 가 없다.** 트라이그램 점수에 임계값을 두면 "찾았는데 안
  보여주는" 일이 생긴다
- 검색어가 짧으면 **`400 KEYWORD_TOO_SHORT`**. 최소 길이는
  `SEARCH_KEYWORD_MIN_LENGTH`(기본 2)이고 환경에서 바뀐다
- **임베딩을 만들지 않는다.** 모델 서버 없이 동작한다 —
  `임베딩 모델 서빙 경로 확정(SYS-002-4)` 이 미결인 상태에서도 실제 검색을 돌려 볼 수 있는 경로다

### `POST /api/search/hybrid` — 결합 (`SRH-004` 키워드·의미 결합)

**화면이 쓰는 것은 이 엔드포인트다.** 나머지 둘은 각각을 재보거나 비교할 때 쓴다.

```
{ "query": str(1~1000), "project_ids": [int]|null, "limit": 1~50 = 10,
  "document_id": int|null, "candidates": 1~200|null }
```

- 순서는 **RRF** — `Σ 1/(k + 순위)` · 순위는 **1부터** · `k = 60`
- 두 점수를 **더하지 않는다.** 스케일이 달라 코사인 `0.8` 과 트라이그램 `0.8` 이
  다른 뜻이기 때문이다. RRF 는 점수를 버리고 순위만 쓴다
- **양쪽에 걸린 결과가 자연히 위로 온다** — 한쪽 1등(`1/61`=0.0164)보다 양쪽
  3등·5등(`1/63+1/65`=0.0313)이 높다
- 검색어가 짧으면 **키워드 쪽만 건너뛰고** 의미 검색으로 답한다. 오류를 내지 않는다
- `candidates` 는 두 방식에서 각각 가져올 후보 수(기본
  `SEARCH_HYBRID_CANDIDATES` = 30). **이 값이 `SRH-002-1 검색 결과 재순위`의 상한을
  정한다** — 리랭커는 받은 후보 안에서만 순서를 바꾼다

## 5. 프롬프트 컨텍스트 조립은 엔드포인트가 없다

초안이 *"`RAG-07` 은 내부 로직이라 엔드포인트가 없다"* 고 한 것은 그대로 맞다.
`프롬프트 컨텍스트 조립(RAG-002-1)` 은 `services/context_assembly.py` 의 순수
로직이고, **아직 부르는 곳이 없다**(부분 구현). 붙이는 것은
`문서 기반 질의응답(CHAT-001)` 과 함께 한다.
...
`unit_price` · `category` 는 `AMT-13` 이 추가하는 컬럼이다(리비전 `0010`).
...
## 7. `document_chunks` 스키마 초안 (리비전 `0011`)
...
| 3 | `0011` 첫 줄에 `CREATE EXTENSION IF NOT EXISTS vector` | 테이블보다 먼저 |
...
| 2 | 리비전 `0011` — 확장 · `document_chunks` | **DB 이미지 pgvector 교체** (팀 합의) |
