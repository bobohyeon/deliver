...
#
# !! 이 값은 CHARS_PER_TOKEN 과 곱해져 실제로는 "몇 자 미만을 흡수하는가"로
#    동작한다. 그래서 비율을 고치면 이 정책도 같이 변한다. 48 은 원래 잘못된
#    비율 1.2 를 전제로 맞춰진 값이었다 (48 x 1.2 = 58자 미만 흡수).
#    비율만 1.89 로 고쳤을 때 흡수 기준이 91자로 1.57배 넓어져,
#    "4. 대금 지급" 처럼 짧은 절이 옆 절에 삼켜지는 회귀가 났다. 실측:
#      비율 1.2  · 48 -> 58자 미만 흡수 · 조각 9개 · "돈은 언제 받나요" 1위 정답 53%
#      비율 1.89 · 48 -> 91자 미만 흡수 · 조각 8개 · 정답이 2위로 밀림(47%)
#      비율 1.89 · 30 -> 57자 미만 흡수 · 조각 9개 · 경계가 첫 줄과 동일
#    30 은 58자 기준을 정확한 비율에서 되살린 값이다 (30 x 1.89 = 57자).
MIN_TOKENS = 30
...
# 글자 하나당 토큰 수를 되돌리는 비율(글자 수 / 이 값 = 추정 토큰 수).
#
# 실측값이다. dragonkue/BGE-m3-ko 의 실제 토크나이저로 조달 문서 867건의 본문을
# 재서 얻은 중앙값이 1.89 다 (2026-08-18, 도구/embed-test). 이전 값 1.2 는
# 실측 전 보수적 추정이었고, 토큰을 과대추정해 청크가 필요보다 짧게 끊겼다.
#   1.2  -> 최대 480토큰 x 1.2  = 576자  -> 실제 약 305토큰 (상한의 30%만 사용)
#   1.89 -> 최대 480토큰 x 1.89 = 907자  -> 실제 약 480토큰
#
# !! CHUNK_MAX_TOKENS 와 짝으로 봐야 한다. 두 값의 곱이 청크의 최대 "글자" 수고,
#    그것을 실제 토크나이저로 재서 임베딩 모델의 max_seq_length 를 넘으면 모델이
#    뒤를 조용히 잘라낸다. 에러가 나지 않아서 알아채기 어렵다.
#      · 우리 설정: max_seq_length = 1024 (도구/embed_server.py 의 DEFAULT_MAX_SEQ)
#      · 모델 기본값은 384 다. 우리가 올려 쓰는 것이고, 정확도 측정도 1024 로 했다.
#      · 480 x 1.89 = 907자. 글자/토큰 비가 0.9 까지 떨어지는 최악의 표(숫자만
#        나열된 산출내역서)에서도 1008 토큰이라 1024 안에 들어온다.
#    그래서 이 값이나 CHUNK_MAX_TOKENS 를 올릴 때는 max_seq_length 를 먼저 확인한다.
