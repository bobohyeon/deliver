# RAG 청킹 · 임베딩 전달 (2026-08-14 작성 · 2026-08-16 머지 완료)
...
> |---|---|
> | `main` | **`efacc89`** |
> | PR #23 | `fix: detect_heading 실측 오탐 수정` — 머지 |
> | PR #24 | `feat: RAG-01 청킹 + RAG-02 임베딩 생성·저장` — 머지 |
> | 브랜치 정리 | `fix/structure-false-positives` · `feat/rag-chunking-embedding` · `feat/rag-document-chunks` 로컬·원격 모두 삭제 |
> | main 코드 검증 | 검사 75개 통과 · 문법 13개 통과 · 실제 문서 3건 문제 0건 |
> | 팀 공유 파일 | `requirements.txt` · `docker-compose.yml` · `Dockerfile` · `.env.example` **변경 0** |
> | 마이그레이션 | `20260814_0014 (head)` 적용 확인 |
>
> **아래 「적용 방법」은 이미 끝난 작업이다.** 참고용으로만 남긴다.
>
> ### 아직 안 한 것
>
> | | 항목 |
> |---|---|
> | 1 | **도커 E2E 미실행** — 업로드→청킹→임베딩→DB 저장을 실제로 돌린 적 없다 |
> | 2 | **`iterative_scan` 실행계획 미확인** — 청크가 생긴 뒤 `EXPLAIN ANALYZE` 로 봐야 한다 |
> | 3 | **`CHARS_PER_TOKEN = 1.2` 실측 안 함** — 실제 토크나이저로 재서 고쳐야 한다 |
> | 4 | **재정님께 메시지 미발송** — 추출 파이프라인에 한 줄 추가 요청 + 정확도 수치 정정 (아래 초안) |
> | 5 | **`TSK` 담당 합의 미완** — 기능명세서 10절이 "담당 미정"이다 |
> | 6 | `LocalEmbeddingClient` 실동작 미확인 — 로컬에 Ollama 없음 |
> | 7 | 로컬 브랜치 `ocr-review-schema` 정리 안 함 (원격은 이미 `gone`) |
> | 8 | `origin/feat/local-llm` — 머지됐지만 세현님 브랜치라 물어보고 지울 것 |
>
> ### 다음 작업
>
> **`RAG-04` 의미 검색 API.** `WHERE project_id = X` 가 같은 테이블 조건이 되게 짠다
> (`iterative_scan` 전제). 이걸 만들면 업로드→청킹→임베딩→검색 전 구간이 이어지고,
> 2번 검증도 함께 된다.
