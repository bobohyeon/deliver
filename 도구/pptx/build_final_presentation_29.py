...
def slide11():
    ...
    # 마지막 값(전체 1,339청크)은 2026-09-10 세현님 확인으로 36.9 -> 38.8 로 갱신했다.
    # 나머지 칸은 재확인받지 않았으므로 인수인계 9-G ⑤-b 표 값을 그대로 쓴다.
    r1=[90.2,86.4,80.8,71.3,58.7,49.5,38.8]
    ...
    metric(p, 1420, 515, "38.8", "전체 1,339 · R@1", color=C["cyan"], suffix="%", width=360)
...
def slide27():
    ...
    items=[
        ("01","검색 확장성","전체 1,339청크 R@1 38.8%","후보 생성·인덱스·리랭킹 재설계",C["blue"]),
...
def apply_content_review(slide_no: int, content: str) -> str:
    ...
    reviewed = {
        ...
        11: {
            ...
            # 날짜를 적지 않는다. 1,339청크 값만 2026-09-10 에 갱신됐고 나머지 칸은
            # 2026-08-20 기록이라, 한 날짜로 묶으면 어느 쪽이든 틀린 설명이 된다.
            "※ 후보 30의 90.2%는 제품 문서 수 제한 정책이 아니라 검색 후보 범위 실험값입니다.": "※ 학습을 마친 모델의 측정값입니다. 문서 수 제한 정책이 아니라 검색 범위 실험입니다.",
        },
        12: {
            "RERANKER": "검색 결과 재정렬",
        ...
        27: {
            ...
            "검색 확장성": "검색 대상이 많을 때",
            "전체 1,339청크 R@1 38.8%": "문서 조각 1,339개에서 첫 결과 정답률 38.8%",
...
def first21_slides():
    ...
    **20번은 여기서 만들지 않는다.** 20번은 세현님이 만든 검색 모델 학습(하드
    ...
    items = [
        ...
        (12, 9, slide9, "김보현"),
        (13, 10, slide10, "김보현"),
        # 14~18 은 제품 기능이고, 검색 규모 실험(slide11)은 19번으로 내렸다.
        # 문제 제기(19) → 모델 선택(20) → 학습(21) → 재정렬(22) 를 한 덩어리로
        # 붙이기 위한 것이다. 5장 뒤에서 해결하면 청중이 문제를 잊는다.
        (14, 13, slide13, "김보현"),
        (15, 14, slide14, "김보현"),
        (16, 15, slide15, "김보현"),
        (17, 16, slide16, "김보현"),
