...
HEADER = ["기능명", "선행 기능", "테스트 절차", "예상 결과", "수행결과", "Y/N", "비고"]

WIDTHS = [(1, 22), (2, 22), (3, 46), (4, 40), (5, 30), (6, 6), (7, 30)]

# 각 열 스타일(STYLES 의 cellXfs 인덱스). 4=위정렬+줄바꿈, 5=가운데정렬+줄바꿈.
COL_STYLE = [4, 4, 4, 4, 4, 5, 4]
...
def parse_md():
    ...
    for line in text.split("\n"):
        ...
        cells = cells[1:-1]
        if len(cells) != 8:
            raise SystemExit(f"칸이 {len(cells)}개다(8개여야 한다 — Y/N 열 포함) — {line[:60]}")
        _testid, feat, prereq, proc, expect, result, yn, note = cells
        ...
        raw_rows.append([fid, fname, prereq, proc, expect, result, yn, note])
...
def build(rows):
    ...
    for fname, prereq_names, proc, expect, result, yn, note in rows:
        cells = [fname, prereq_names, proc, expect, result, yn, note]
        sheet.append((None, [(c, v, COL_STYLE[c - 1]) for c, v in enumerate(cells, start=1)]))

    last = len(sheet)
    return sheet_xml(sheet, widths=WIDTHS, freeze=("A5", 0, 4), autofilter=f"A4:G{last}")
...
def main():
    ...
    out_rows = []
    for fid, fname, prereq, proc, expect, result, yn, note in raw_rows:
        out_rows.append([
            clean(fname),
