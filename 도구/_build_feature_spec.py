...
AREAS = [
    ...
    ("H. 태스크·협업", "TSK", "미정", [
        ("TSK-01", "태스크 CRUD", "신규", "P1", "미정", "PRJ-01", "제목·설명·담당자·기한·상태를 사람이 직접 만들 수 있다", ""),
        ("TSK-02", "칸반 보드", "신규", "P1", "미정", "TSK-01", "할 일·진행 중·완료 3열, 열별 개수 표시, 드래그로 상태 변경", ""),
        ("TSK-03", "AI 제안 → 태스크 확정", "신규", "P1", "미정", "ANL-03", "승인해야 등록된다. 승인 전에는 보드에 나타나지 않는다", "자동 등록 금지 원칙"),
        ("TSK-04", "출처 · 판단 근거 표시", "신규", "P1", "미정", "TSK-03", "태스크에 출처 문서 링크와 AI 판단 근거(reason)가 함께 보인다", "원문 위치 하이라이트는 범위 밖"),
        ("TSK-05", "AI 생성 배지 · 필터", "신규", "P1", "미정", "TSK-03", "AI 생성 태스크가 구분 표시되고 AI생성만·기한임박·출처문서별 필터가 동작한다", ""),
        ("TSK-06", "담당자 · 기한 지정", "신규", "P1", "미정", "TSK-01", "AI 가 못 찾은 경우 미지정 상태로 남고 사람이 채운다", ""),
        ("TSK-07", "제안 거부", "신규", "P1", "미정", "TSK-03", "거부한 제안은 다시 뜨지 않는다. 거부 사실이 기록된다", "채택률 지표의 원천"),
        ("TSK-08", "일괄 승인", "신규", "P1", "미정", "TSK-03", "문서 단위로 여러 제안을 한 번에 승인할 수 있다", ""),
        ("TSK-09", "활동 로그", "신규", "P2", "미정", "PRJ-01", "", "activity_logs. 주간 보고서 재료"),
        ("TSK-10", "알림 (이메일 · Slack)", "신규", "P3", "미정", "TSK-09", "", ""),
    ]),
    ("I. 가시성", "VIS", B, [
    ...
    ("J. 공통·기반", "SYS", "미정", [
        ("SYS-01", "Alembic 마이그레이션", "신규", "P0", "미정", "", "create_all() 의존을 제거한다. 컬럼 추가가 데이터 유실 없이 반영된다", "P0 착수 전 완료"),
        ("SYS-02", "Celery + Redis", "신규", "P0", J, "", "워커가 죽어도 작업이 큐에 남고 재시작 시 이어진다", ""),
        ("SYS-03", "에러코드 체계", "확장", "P0", "미정", "", "기존 11종에 인증·권한·프로젝트 코드를 추가. 응답 형식이 일관된다", ""),
        ("SYS-04", "request_id 로깅", "기존", "P0", "미정", "", "요청 단위로 로그 추적 가능", ""),
        ("SYS-05", "Fake AI Client", "기존", "P0", "미정", "", "API 키 없이 전체 흐름이 동작한다", "병렬 작업의 전제"),
        ("SYS-06", "API 호출 쿼터", "신규", "P2", "미정", "", "", "프로젝트별 월 한도"),
        ("SYS-07", "N+1 방지", "신규", "P0", "미정", "", "목록 조회 쿼리 수가 문서 수와 무관하게 일정하다. selectinload 사용", "미니 2+2N 해소"),
        ("SYS-08", "FK 인덱스", "신규", "P0", "미정", "SYS-01", "모든 FK 에 명시적 인덱스. PostgreSQL 은 자동 생성하지 않는다", ""),
        ("SYS-09", "에러 로그 누락 수정", "확장", "P0", "미정", "", "404·409·413 이 서버 로그에 남는다", "미니 ISS-046. 첫날 수정"),
        ("SYS-10", "로컬 sLLM 옵션", "신규", "P3", "미정", "", "", ""),
    ]),
    ("K. 산출물", "DLV", B, [
