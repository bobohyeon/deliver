...
AREAS = [
    ...
    ("B. 프로젝트", "PRJ", S, [
        ...
        ("PRJ-02", "내 프로젝트 목록", "신규", "P0", S, "PRJ-01", "내가 멤버인 프로젝트만 나온다. 카드에 문서 수·열린 태스크·승인 대기 표시", ""),
    ...
    ("C. 문서 입력·처리", "DOC", J, [
        ...
        ("DOC-11", "문서 삭제", "기존", "P0", B, "", "하위 데이터(추출텍스트·분석·OCR요소·금액·결정·일정)가 함께 정리된다", "cascade"),
        ("DOC-12", "재분석", "신규", "P1", B, "ANL-07", "기존 분석을 지우지 않고 새 행으로 쌓인다. 이전 결과와 비교 가능", "analyses 1:N 의 효과"),
        ("DOC-13", "중복 파일 검사 (SHA-256)", "신규", "P2", J, "", "", ""),
        ("DOC-14", "파일 저장소 이전 (S3·MinIO)", "신규", "P1", S, "", "컨테이너 재기동 후에도 파일이 유지된다", "산출물 파일도 같은 곳에 둔다"),
    ...
    ("F. 분석", "ANL", B, [
        ("ANL-01", "요약 분석기", "기존", "P0", B, "", "요약문과 토큰·지연시간이 기록된다", "코드를 고치지 않는다"),
        ("ANL-02", "분류 분석기", "확장", "P0", B, "DOC-16", "7종 중 하나로 분류하고 reason 을 함께 반환한다. 자동 판별인 문서에만 실행한다", ""),
        ("ANL-03", "항목 추출 분석기", "신규", "P1", B, "", "액션아이템·결정·일정을 한 번의 호출로 배열로 반환. 각 항목에 reason 과 확신도가 붙는다", "제안 골격의 첫 사례"),
        ("ANL-05", "분석 진행률 일반화", "신규", "P1", B, "", "진행 단계가 분석기 수에 따라 자동으로 '분석 3/4' 처럼 표시된다. 분석기 추가 시 화면 코드를 고치지 않는다", "목업 4단계 하드코딩 해소"),
        ...
        ("ANL-08", "프롬프트 버전 관리", "기존", "P0", B, "", "prompt_version 이 결과와 함께 저장된다", "분석기별로 따로 올린다"),
        ("ANL-09", "LLM 재시도 · 폴백", "신규", "P2", B, "", "일시 실패 시 재시도, 지속 실패 시 대체 모델", "현재 타임아웃만 있다"),
        ("ANL-10", "제안 종류별 채택률", "신규", "P2", B, "TSK-07", "채택률이 액션아이템·결정·일정·금액별로 나뉘어 표시된다", "목업은 단일 숫자"),
        ("ANL-11", "교정 분석기", "신규", "P3", B, "", "", ""),
        ("ANL-12", "번역 분석기", "신규", "P3", B, "", "", ""),
        ("ANL-14", "결정사항 추출", "신규", "P1", B, "ANL-03", "회의록에서 결정된 것을 뽑아 decisions 에 쌓는다. 결론이 안 난 것은 PENDING 으로 남아 회의 안건이 된다", "DLV-05·DLV-06 의 재료"),
        ("ANL-15", "일정 · 기한 추출", "신규", "P1", B, "ANL-03", "문서의 날짜를 뽑아 schedule_items 에 쌓는다. 날짜는 형식이 있어 원문 대조로 정확도를 잴 수 있다", "DLV-04·DLV-07 의 재료"),
    ]),
    ("G. 금액", "AMT", B, [
        ("AMT-01", "금액 항목 추출", "신규", "P1", B, "", "항목명·수량·단위·금액·reason 을 구조화 스키마로 반환. 문서에 없는 값은 비운다", "LLM 은 추출만. 계산은 코드"),
        ...
        ("AMT-09", "금액 판단 근거 표시", "신규", "P1", B, "AMT-01", "금액 항목마다 reason 이 보이고 출처 문서로 이동할 수 있다", "위치 하이라이트는 범위 밖"),
    ...
    ("H. 태스크·협업", "TSK", S, [
        ...
        ("TSK-04", "출처 · 판단 근거 표시", "신규", "P1", S, "TSK-03", "태스크에 출처 문서 링크와 AI 판단 근거(reason)가 함께 보인다", "원문 위치 하이라이트는 범위 밖"),
        ("TSK-05", "AI 생성 배지 · 필터", "신규", "P1", S, "TSK-03", "AI 생성 태스크가 구분 표시되고 AI생성만·기한임박·출처문서별 필터가 동작한다", ""),
        ...
        ("TSK-09", "활동 로그", "신규", "P2", S, "PRJ-01", "", "activity_logs. 주간 보고서 재료"),
        ("TSK-10", "알림 (이메일 · Slack)", "신규", "P3", S, "TSK-09", "", ""),
    ]),
    ("I. 가시성", "VIS", ALL, [
        ("VIS-01", "대시보드 지표 카드", "신규", "P2", ALL, "PRJ-01", "", "전체 문서·처리 중·열린 태스크·승인 대기"),
        ("VIS-02", "문서 유형 분포", "신규", "P2", ALL, "ANL-02", "", ""),
        ("VIS-03", "승인 대기 배너", "신규", "P1", ALL, "TSK-03", "승인 대기 건수와 출처 문서명이 보이고 검토 화면으로 이동한다", "제안 종류별로 나눠 표시"),
        ("VIS-04", "최근 문서", "신규", "P2", ALL, "DOC-08", "", ""),
        ("VIS-05", "이번 주 활동", "신규", "P2", ALL, "ANL-10", "", "제안 종류별 채택률 포함"),
