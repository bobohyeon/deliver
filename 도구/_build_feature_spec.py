...
AREAS = [
    ...
    ("C. 문서 입력·처리", "DOC", J, [
        ...
        ("DOC-16", "업로드 시 문서 유형 지정 (기본값 자동 판별)", "신규", "P1", J, "DOC-01", "유형을 고르면 분류 분석기를 건너뛴다. 자동 판별을 고르면 분류 분석기가 먼저 실행되고 결과가 확정값으로 저장된다", "일괄은 배치 단위로 한 번 지정. ANL-04 의 전제"),
    ]),
    ("D. OCR 검수", "REV", J, [
        ...
        ("REV-17", "텍스트 재조립", "신규", "P1", J, "REV-05", "박스 수정 후 extracted_texts.content 가 같은 트랜잭션에서 갱신된다", "offset 기록 불필요 — ANL-13 폐기"),
    ...
    ("F. 분석", "ANL", B, [
        ...
        ("ANL-02", "분류 분석기", "확장", "P0", B, "DOC-16", "자동 판별을 고른 문서에만 실행된다. 7종 중 하나로 분류하고 reason 을 함께 반환한다", "사람이 유형을 고르면 건너뛴다"),
        ("ANL-03", "액션아이템 분석기", "신규", "P1", B, "ANL-04", "내용·담당자·기한·확신도·reason(왜 이걸 뽑았는지)을 배열로 반환. 담당자·기한을 못 찾으면 미지정 표시", "제안 골격의 첫 사례"),
        ("ANL-04", "분석기 선택 규칙", "신규", "P1", B, "DOC-16", "문서 유형에 따라 실행할 분석기가 결정된다. 새 분석기 추가 시 규칙 표에 행 하나 추가로 끝난다", "analyzer_rules 테이블. FastAPI 라우터와 무관"),
    ...
    ("G. 금액·비용추계", "AMT", B, [
        ("AMT-01", "금액 항목 추출", "신규", "P1", B, "ANL-04", "항목명·수량·단위·단가·금액·reason(왜 이 항목으로 봤는지)을 구조화 스키마로 반환. 스키마에 없는 필드는 나올 수 없다", "문서에 없는 값은 비운다"),
        ...
        ("AMT-09", "금액 판단 근거 표시", "신규", "P1", B, "AMT-01", "금액 항목마다 reason 이 보이고 출처 문서로 이동할 수 있다", "위치 하이라이트는 범위 밖. 검증은 AMT-03"),
    ...
    ("H. 태스크·협업", "TSK", S, [
        ...
        ("TSK-04", "출처 · 판단 근거 표시", "신규", "P1", S, "TSK-03", "태스크에 출처 문서 링크와 AI 판단 근거(reason)가 함께 보인다. 링크를 누르면 문서 상세로 이동한다", "원문 위치 하이라이트는 범위 밖"),
...
def build_summary_sheet():
    ...
    rows.append((32, [(1, "구현 20일 (8/17~9/5 · 영업일 15일 · 3인 45 인일). "
                          "P0+P1 을 다 못 하므로 '묶기' 로 접근한다. "
                          "근거 페이지 표시를 범위에서 빼면서 ANL-13(분석기 규격 확장)이 "
                          "폐기되어 세 사람의 인터페이스 대기가 하나 줄었다.", S_NOTE)]))
...
ROUTES = [
    ("계약서 CONTRACT", "O", "자동일 때만", "조항 검토", "O — 계약금액", "contract_v1"),
    ("계약변경 CONTRACT_CHANGE", "O", "자동일 때만", "조항 검토", "O — 증감액", "change_v1"),
    ("회의록 MEETING_NOTES", "O", "자동일 때만", "O", "비용 발생 요인만", "meeting_v1"),
    ("보고서 REPORT", "O", "자동일 때만", "O", "진행률 대비", "report_v1"),
...
def build_route_sheet():
    rows = []
    rows.append((22, [(1, "분석기 선택 규칙 (ANL-04)", S_TITLE)]))
    rows.append((46, [(1, "문서 유형에 따라 실행할 분석기를 정한다. FastAPI 라우터와는 "
                          "무관한 층이다. 유형은 업로드할 때 정해진다(DOC-16) — "
                          "사람이 고르면 분류 분석기를 건너뛰고, 자동 판별을 고르면 "
                          "분류만 먼저 실행해 확정값을 만든다. 이렇게 해야 '분류는 "
                          "분석 결과인데 선택은 분석 전' 인 순환이 풀린다.", S_NOTE)]))
    rows.append((None, []))
    hdr = ["문서 유형 (document_type) — 7종", "요약", "분류", "액션아이템", "금액",
           "prompt_variant"]
    rows.append((26, [(i + 1, h, S_HDR) for i, h in enumerate(hdr)]))
    for r in ROUTES:
        rows.append((None, [
    ...
    rows.append((None, []))
    rows.append((60, [(1, "prompt_variant 가 필요한 이유 — 같은 분석기라도 문서 유형에 "
                          "따라 프롬프트가 달라야 한다. 액션아이템은 회의록(담당자·기한 "
                          "중심)과 계약서(조항 검토 중심)가 다르고, 금액은 계약서(계약금액)와 "
                          "계약변경(증감액)이 다르다. 계약변경을 별도 유형으로 나눈 이유가 "
                          "이것이다 — 프롬프트가 스스로 판단하게 하면 그게 또 하나의 분류 "
                          "문제가 되고 틀렸을 때 사람이 고칠 방법이 없다.", S_NOTE)]))
    return sheet_xml(rows, [(1, 34), (2, 10), (3, 13), (4, 18), (5, 22),
                            (6, 18)])
...
OPEN = [
    ...
    ("결정됨", 4, "근거 페이지·좌표 하이라이트 범위 제외",
     "확정 — reason(판단 근거 서술)만 표시. ANL-13 폐기", "전원"),
    ("결정됨", 5, "문서 유형을 업로드 시 지정 (기본값 자동 판별)",
     "확정 — DOC-16. 일괄은 배치 단위 지정", "전원"),
    ("설계", 6, "파이프라인 단계 일반화 (ANL-05)",
     "목업·프론트·enum·백엔드 4곳이 걸린다", "세현"),
    ("결정됨", 7, "문서 유형을 7종으로 (계약변경 추가)",
     "확정 — 원계약은 계약금액, 변경합의서는 증감액. 프롬프트가 다르다", "전원"),
    ("설계", 8, "분류가 틀렸을 때 사람이 document_type 을 고칠 수 있게 할지",
     "허용. document_type_source=USER_CORRECTED 로 기록하면 분류 오류율이 나온다", "세현"),
    ("설계", 9, "tasks 출처를 nullable FK 2개로 둘지",
     "FK 무결성을 DB 가 보장한다. source_type+source_id 는 못 건다", "세현"),
    ("설계", 10, "제안 거부를 result_json.decision 에 둘지 별도 테이블로 둘지",
     "채택률 지표의 원천", "보현"),
    ("설계", 11, "analyzer_rules 를 테이블로 둘지 코드 상수로 둘지",
